...
const int MAX_SINH_VIEN = 100; // Số lượng sinh viên tối đa
...
void NhapThongTin(SinhVien danhSach[], int& n) {
    ...
    if (n > MAX_SINH_VIEN) {
        ...
        n = MAX_SINH_VIEN; // Cắt số lượng xuống tối đa
    }
...
void SapXepChon(SinhVien danhSach[], int n) {
    for (int i = 0; i < n - 1; i++) {
        int maxIndex = i; // Thay minIndex thành maxIndex để sắp xếp theo điểm
        for (int j = i + 1; j < n; j++) {
...
void Ghep(SinhVien danhSach[], int left, int mid, int right) {
    ...
    while (i < n1 && j < n2) {
        if (leftList[i].DiemTrungBinh >= rightList[j].DiemTrungBinh) { // Sắp xếp giảm dần
...
int YenCau(SinhVien danhSach[], int left, int right) {
    ...
    for (int j = left; j < right; j++) {
        if (danhSach[j].DiemTrungBinh > pivot.DiemTrungBinh) { // Sắp xếp giảm dần
            i++;
